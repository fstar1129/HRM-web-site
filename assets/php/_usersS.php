...
// Member Number
$content = $html->div(array('content' => 'Member #', 'class' => 'label float-left'));
$content .= $html->input(array('id' => 'membernumber', 'class' => 'float-left'));
...
/*/* Division
$divisions = array();
$divisions[''] = 'Please select..'; 
$db = new db('usersdivision', 'eris_eris');
$db->select(false, 'name ASC', false, array());
while($db->getRow()) {
    $divisions[$db->code] = $db->name; 
}

$content = $html->div(array('content' => 'Division', 'class' => 'label float-left'));
$content .= $html->select(array('id' => 'division', 'class' => 'float-left', 'options' => $divisions)).$clear;
$rightContent .= $html->fieldset(array('content' => $content));
*/

// Account Type
$content = $html->div(array('content' => 'Account Type', 'class' => 'label float-left'));    
$sectors = array('' => 'Please select..', 'S' => 'Super User', 'M' => 'Master', 'P' => 'Parent', 'C' => 'Child');
